...
def getTrainedModel(corpusDir='D:/linuxDownloads/iweb/' ):
    ...
    # corpusData = getCorpusData(corpusdir='D:\\linuxDownloads\\text0 (copy)')
    corpusData = getCorpusData(corpusdir=corpusDir)
    ...
    if(hashSame and os.path.isfile(os.path.join(dir_path,'trainedCorpusHashIwebGensim.joblib'))):
        ...
        trainedModel = joblib.load(os.path.join(dir_path,'trainedCorpusHashIwebGensim.joblib'))
        ...
        joblib.dump(trainedModel, os.path.join(dir_path,'trainedCorpusHashIwebGensim.joblib'))
        print('dumped model at: ' + str(datetime.datetime.now()))

    (tfidf, dictionary, bow) = trainedModel
    return (tfidf, dictionary, bow)
